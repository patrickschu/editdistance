...
#@timer
def variantfinder_words(input_dict, meta_data, variant_one, variant_two, token_threshold):
	"""
...
def main(input_dir, output_file, variant_one, variant_two, token_threshold):
	"""
	Takes data from input_dir, a folder with subfolders of text files. 
	Writes resulting CSV to output_file. 
	Investigates variation btw variant_one and variant_two, see variantfinder_words for details.
	Also outputs totalwords.json, which gives dictionary size of corpus
	and variablewords.json, which gives counts for words that vary by variant_one and variant_two. 
	"""		
	input_dir= input_dir
	output_file= output_file
	t=emod.dictbuilder_2(input_dir, 'pubdate')
	#print "t keys", t.keys()
	u, totaldict=variantfinder_words(t, 'meta_data placeholder', 'u','v', int(token_threshold))
	with codecs.open("variablewords.json", "w", "utf-8") as finalout:
		json.dump(u, finalout)
	with codecs.open("totalwords.json", "w", "utf-8") as totalout:
		json.dump(totaldict, totalout)
